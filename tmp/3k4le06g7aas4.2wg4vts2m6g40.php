...
<head>
    ...
    <link rel="stylesheet" href="<?= ($BASE) ?>/UI/css/book.css">
</head>
<body>
...
<!--
    <form method="POST" action="search">
      <label>Введите название книги или произведения.</label>
        ...
        Найдено <?= ($count) ?> книг(а) по запросу <?= ($query) ?><br><hr>
        <div class="container book">
              <?php $i=0; foreach (($books?:[]) as $row): $i++; ?>
                  <p><?= ($i) ?><a href="<?= ($BASE) ?>/book=<?= ($row['BOOK_ID']) ?>"><?= ($row['BOOK_NAME']) ?></a></p>
              <?php endforeach; ?>
        </div>
-->
  </div>
